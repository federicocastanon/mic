	...
	*/
	function get_password_hash($email){
		...
		$sts = $this->CI->db
			...
			->update('users', array('forgot_password_hash' => $hash));
		return $hash;
	}

	function clear_password_hash($email) { 
		$sts = $this->CI->db
			->where(array('email' => $email))
			->update('users', array('forgot_password_hash' => ''));
	}

	function get_verify_email_hash($id){
		$hash = md5(uniqid());
		// update the database
		$sts = $this->CI->db
			->where(array('id' => $id))
			->update('users', array('confirm_email_hash' => $hash));
		return $hash;
	}

	function verify_email($hash) { 
		$query = $this->CI->db
			->where(array('confirm_email_hash' => $hash))
			->get('users');
		if ($query->num_rows() > 0) { 
			$query = $this->CI->db
			->where(array('confirm_email_hash' => $hash))
			->update('users', array('confirm_email_hash' => '', 'status' => 'ACTIVE'));
			return true;
		}
		return false;
	}

	function verify_password_hash($email, $hash) { 
		$query = $this->CI->db
			->where(array('email' => $email, 'forgot_password_hash' => $hash))
			->get('users');
		if ($query->num_rows()) { 
			return $query->row()->id;
		} else { 
