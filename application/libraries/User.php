...
class User {
	...
	*/
	function login($login, $password, $update_last_login = true, $hashed_password = false){
		$user_query = $this->CI->db->get_where('users', array('login'=>$login, 'deleted' => 0));
		#print_r($user_query);die('aca');
		#if (strlen($login)) { print_r($user_query);echo $this->CI->db->last_query();die("login = $login"); }
	...
	*/
	function update_pw($new_pw){
 		// updates the session
		$hashed_password = $this->CI->bcrypt->hash($new_pw);
		$this->CI->session->set_userdata(array('pw'=>$hashed_password));
		$this->user_data->password = $hashed_password;
